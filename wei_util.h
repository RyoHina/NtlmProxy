...
int wei_util_split(IN OUT char * buf,IN char * ch, IN int num ,OUT char * * dst);
/** 检查是否都是数字*/
int  wei_util_check_allnum(IN char * a);
/** 去除字符串的前后空格 */
void wei_util_str_trim(IN OUT char * a);
/** 字符串之间的比较，可选择是否大小写敏感，以及设定比较的长度，如果为长度<=0，表示全字符串比较。*/
int  wei_util_str_compare(IN const char * a, IN const char * b,IN bool isCase,IN int length);
/** a忽略前后空格和b进行比较*/
int  wei_util_str_compare_trim(char * a, char * b,bool is_case);
#endif
#ifdef __cplusplus
}
#endif
