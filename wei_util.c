...
/*出现某个字符串的次数*/
int wei_util_get_str_num(char* buf, char* ch){
    int num=0;
    char* a=buf, *b=NULL;
...
int wei_util_split(char* buf, char* ch,int num ,char** dst){
    char* a=buf,*b=buf;
    int i;
    for(i=0;i<num;i++){
	b=strstr(a,ch);
...
int wei_util_str_compare(const char* a,const char* b ,int isCase,int length){
	...
	return length<=0 ? strcasecmp(a,b)==0:strncasecmp(a,b,length)==0;
    if(isCase)
	return length<=0 ? strcmp(a,b)==0:strncmp(a,b,length)==0;
    int len_a=strlen(a);
    int len_b=strlen(b);
...
/*b忽略前后空格和a比较*/
int wei_util_str_compare_trim(char* a, char* b ,int is_case){
    char* h=a;
    char* e=a+strlen(a)-1;
    while(*h==' ')
	h++;
